...
if os.path.isfile(generator_path):
	...
	for epoch in range(args.num_epochs):
		...
		while iteration < total_step:
            ...
            valid = autograd.Variable(Tensor(real_data.shape[0]).fill_(1.0), requires_grad=False)
            fake = autograd.Variable(Tensor(real_data.shape[0]).fill_(0.0), requires_grad=False)
